...
async def select_language():
    ...
    return language 
async def agri_nex_main_flow(language):
    ...
    if not user_choice:
        ...
        return await agri_nex_main_flow(language)
...
async def register_farmer_voice(language):
...
async def handle_farmer_interaction(phone_number, language,skip_phone_prompt=False):
        ...
        if phone:
            ...
            if farmer:
                ...
                while True:
                    ...
                    if user_choice in options:
                        ...
                        if user_choice == "1":
                            ...
                            await sell_commodities(farmer, language)
                        elif user_choice == "4":
                            await request_help(farmer, language)
                        elif user_choice =="5":
                            await emergency_alert_number_menu(farmer,farmer_id, language)
                        elif user_choice == "6":
                            await speak_translated("Starting Agrinex AI Assistant. Speak naturally in your language.", language)
                            await bulletproof_voice_menu(language)

                        await return_or_exit(language, phone)
                        return phone, farmer
                    else:
                ...
                if response and "register" in response.lower():
                    await register_farmer_voice(language)
                    return
...
async def sell_commodities(farmer,language):
...
async def request_help(farmer_id, language):
...
async def emergency_alert_number_menu(farmer,farmer_id, language):
...
async def return_or_exit(language, phone_number):
    ...
    while True:
        ...
        if response in ["exit", "quit", "stop", "number zero"]:
            ...
            await handle_farmer_interaction(phone_number, language,skip_phone_prompt=True)
            return
        elif number == 0:
            await speak_translated("Thank you for using AgriNex. Goodbye!", language)
            return
...
async def bulletproof_voice_menu(language):
    ...
    if not response:
        ...
        return await bulletproof_voice_menu(language)
...
async def main():
    # Language Selection
    language = await select_language()  # Choose language
        ...
        if not phone_number:
            ...
            if user_input:
                ...
                if user_input in ["register", "sign up", "number one", "one", "1", "number 1"]:
                    ...
                    phone_number = await register_farmer_voice(language)

                    if phone_number:  # Registration successful
                        await handle_farmer_interaction(phone_number, language)
                        continue
                # Handle existing users
                elif user_input in ["number two", "two", "2", "number 2"]:
                    await speak_translated("Please say your 10-digit phone number.", language)
                    phone_number, farmer = await handle_farmer_interaction(phone_number,language)
